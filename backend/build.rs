...
/// Generate Rust code that embeds all assets using include_bytes!
fn generate_assets_code(dist_dir: &Path) {
    use std::io::Write;

    let out_dir = std::env::var("OUT_DIR").unwrap();
    let dest_path = PathBuf::from(out_dir).join("assets.rs");
    let mut f = fs::File::create(&dest_path).expect("Failed to create assets.rs");
    ...
    writeln!(
        f,
        "pub fn get_assets() -> HashMap<&'static str, EmbeddedAsset> {{"
    )
    .unwrap();
    writeln!(f, "    let mut assets = HashMap::new();").unwrap();

    if dist_dir.exists() {
        collect_files(dist_dir, dist_dir, &mut f);
    }

    writeln!(f, "    assets").unwrap();
    writeln!(f, "}}").unwrap();

    println!("cargo:rerun-if-changed={}", dest_path.display());
}

/// Recursively collect all files and generate include_bytes! for them
fn collect_files(base_dir: &Path, current_dir: &Path, f: &mut fs::File) {
    use std::io::Write;
    ...
    if let Ok(entries) = fs::read_dir(current_dir) {
        for entry in entries.filter_map(|e| e.ok()) {
            ...
            if path.is_file() {
                ...
                // Generate the include_bytes! line
                writeln!(
                    f,
                    "    assets.insert(\"{}\", EmbeddedAsset {{ data: include_bytes!(concat!(env!(\"CARGO_MANIFEST_DIR\"), \"/dist/{}\")) }});",
                    path_str,
                    path_str
                ).unwrap();
            } else if path.is_dir() {
                collect_files(base_dir, &path, f);
            }
        }
    }
}
